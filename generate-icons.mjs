...
const CTX_LABEL = {actions: 'Actions', status: 'Status', devices: 'Devices',
    places: 'Places', mimetypes: 'MimeTypes', emblems: 'Emblems',
    categories: 'Categories', apps: 'Applications'};

// Lucide inventory straight from the package (no worklist.json needed).
const LU = new Set(readdirSync(LUCIDE_DIR)
...
let agentApplied = 0, agentInvalid = 0;
...
if (existsSync(VENDORED) && !existsSync(`${ICONS}/out`)) {
    ...
    for (const f of readdirSync(`${ICONS}/chunks`)) {
        const {context, icons} = JSON.parse(readFileSync(`${ICONS}/chunks/${f}`, 'utf8'));
    ...
    if (existsSync(`${ICONS}/out`)) {
        for (const f of readdirSync(`${ICONS}/out`)) {
            ...
            let data;
            try { data = JSON.parse(readFileSync(`${ICONS}/out/${f}`, 'utf8')); }
            catch { console.warn(`  skip malformed ${f}`); continue; }
            ...
            for (const mm of data.mappings || []) {
                ...
                if (LU.has(mm.lucide)) { finalMap[ctx][mm.name] = mm.lucide; agentApplied++; }
...
const GENERIC = {
    mimetypes: {
        ...
        'text-x-generic': 'file-text', 'image-x-generic': 'file-image',
        'audio-x-generic': 'file-audio', 'video-x-generic': 'file-video',
        'package-x-generic': 'package', 'font-x-generic': 'type',
        'application-octet-stream': 'file', 'unknown': 'file-question', 'gtk-file': 'file',
        'application-x-archive': 'file-archive', 'application-pdf': 'file-text',
    },
    places: {
        'folder': 'folder', 'inode-directory': 'folder', 'folder-remote': 'folder-symlink',
        'user-home': 'house', 'user-desktop': 'monitor', 'user-trash': 'trash-2',
        'network-server': 'server', 'network-workgroup': 'network', 'start-here': 'house',
    },
    devices: {
        'drive-harddisk': 'hard-drive', 'drive-removable-media': 'usb', 'computer': 'monitor',
        'media-optical': 'disc', 'input-keyboard': 'keyboard', 'input-mouse': 'mouse',
    },
    apps: {'application-default-icon': 'app-window', 'application-x-executable': 'square-chevron-right'},
    status: {'image-missing': 'image-off', 'image-loading': 'loader', 'dialog-error': 'circle-x'},
