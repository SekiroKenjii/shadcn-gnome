...
// the originals once). The metric->lucide map lives in assets/icon-overrides.json.
// Usage: node lib/patch-vitals.mjs [VITALS_EXT_DIR]
import {
  cpSync,
  existsSync,
  readdirSync,
...
import { loadIconOverrides, lucideDir } from './config.mjs';

const LUCIDE = lucideDir();
const MAP = loadIconOverrides().vitals || {};
const VITALS =
  process.argv[2] ||
...
// stroke 1.25, matching the system symbolic icons.
const lucideOutline = (n) =>
  outline(readFileSync(`${LUCIDE}/${n}.svg`, 'utf8'), {
    pad: 0,
    strokeWidth: 1.25,
  });
...
for (const set of ['original', 'gnome']) {
  ...
  for (const f of readdirSync(dir)) {
    ...
    const lu = MAP[base];
    if (!lu) {
      console.warn(`  no mapping for ${set}/${base}`);
      continue;
    }
