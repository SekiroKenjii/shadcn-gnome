...
/* Header bar, toolbar and menu buttons follow the GNOME flat convention; the
   raised outlined look is only for standalone content buttons. The extra
   :not()s raise specificity above the outlined rule above so this wins. */
:is(headerbar, .titlebar, windowhandle, .toolbar, menubutton) button:not(.flat):not(.image-button):not(.suggested-action):not(.destructive-action) {
  border: none;
  background-color: transparent;
  box-shadow: none;
}
:is(headerbar, .titlebar, windowhandle, .toolbar, menubutton) button:not(.suggested-action):not(.destructive-action):hover {
  background-color: ${d.hover};
}
:is(headerbar, .titlebar, windowhandle, .toolbar, menubutton) button:not(.suggested-action):not(.destructive-action):checked,
:is(headerbar, .titlebar, windowhandle, .toolbar, menubutton) button:not(.suggested-action):not(.destructive-action):active {
  background-color: ${d.activeSel};
}
...
#panel, #panel:overview { background-color: ${d.headerbar}; }
#panel .panel-button, #panel .panel-button:hover { color: ${d.fg}; }
...
.modal-dialog { background-color: ${d.card}; color: ${d.fg}; }
.button, .login-dialog-button, .modal-dialog-button, .login-dialog-session-list-button {
  background-color: ${d.secondary};
  color: ${d.fg};
  border: 1px solid ${d.border};
  border-radius: ${RADIUS}px;
}
.button:hover, .login-dialog-button:hover, .modal-dialog-button:hover { background-color: ${d.hover}; }
.button:active, .button:checked,
.login-dialog-button:active, .modal-dialog-button:default {
  background-color: ${d.activeSel};
}
`;
}
...
out('gdm/gnome-shell-gdm.css', gdm(d));
